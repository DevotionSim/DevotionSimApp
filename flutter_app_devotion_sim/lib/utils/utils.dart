...
Future<List<Stats>?> jsonStats(String source) async {
  final my_data =
      jsonDecode(await getJson(source)).cast<Map<String, dynamic>>();
  return my_data.map<Stats>((json) => Stats.fromJson(json)).toList();
}

// Method to transform .json in StatsList
Future<StatsList?> jsonStatsList(String source) async {
  final my_data = jsonDecode(await getJson(source));
  return StatsList.fromJson(my_data);
}
...
Future<Stats> fetchStats(http.Client client) async {
  final response = await client.get(Uri.parse(
      'https://drive.google.com/file/d/1K_uMpXeOd1bkv-OWnNj22p-VzWkstApw/view'));
